...
__host__ void insert_help(Node *node, Node *newChild, int newSmallest);

__host__ Node *createNode(int data = -1)
{
  Node *n = new Node;
...
__host__ bool isLeaf(Node *node)
{
  return (node->child[0] == NULL);
}


__host__ int getSmallest(Node *node)
{
...
__host__ int sibNumber(Node *node)
{
  for (int i = 0; i < 3; ++i) {
...
__host__ void updateParentSmallest(Node *node, int data)
{
...
__host__ void insert1Siblings(Node *node, Node *newChild, int newSmallest)
{
...
__host__ void insert2Siblings(Node *node, Node *newChild, int newSmallest)
{
  int newKey = newChild->key[0];
  newChild->parent = node;
...
__host__ void insert3Siblings(Node *node, Node *newChild, int newSmallest) {
  int newKey = newChild->key[0];
  int splitSmallest = -1;
...
__host__ Node* findSpot(Node *node, int data) {
...
__host__ void insert(Node *node, int data)
{
